...
/**
 An object that includes parsed VAST values that are coming from ADCInterstitial class, so 
 we don't need to parse it again from server.
 */
@property(nonatomic,strong) ADCParser *parsed;
...
/**
 Called in ADCInterstitial to give parsed item to ADCVideo class.
 */
-(void)takeParsed:(ADCParser *)parsed;
@end
